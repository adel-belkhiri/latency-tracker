...
void _cds_lfht_gc_bucket(struct cds_lfht_node *bucket, struct cds_lfht_node *node)
{
	...
	BUG_ON(!(!is_bucket(bucket)));
	BUG_ON(!(!is_removed(bucket)));
	BUG_ON(!(!is_removal_owner(bucket)));
	BUG_ON(!(!is_bucket(node)));
	BUG_ON(!(!is_removed(node)));
	BUG_ON(!(!is_removal_owner(node)));
	for (;;) {
		iter_prev = bucket;
		/* We can always skip the bucket node initially */
		iter = rcu_dereference(iter_prev->next);
		BUG_ON(!(!is_removed(iter)));
		BUG_ON(!(!is_removal_owner(iter)));
		BUG_ON(!(iter_prev->reverse_hash <= node->reverse_hash));
		/*
		...
		for (;;) {
			if (unlikely(is_end(iter)))
				return;
			if (likely(clear_flag(iter)->reverse_hash > node->reverse_hash))
				return;
		...
		struct cds_lfht_node *new_node)
{
	...
	BUG_ON(!(!is_removed(old_node)));
	BUG_ON(!(!is_removal_owner(old_node)));
	BUG_ON(!(!is_bucket(old_node)));
	BUG_ON(!(!is_removed(new_node)));
	BUG_ON(!(!is_removal_owner(new_node)));
	BUG_ON(!(!is_bucket(new_node)));
	BUG_ON(!(new_node != old_node));
	for (;;) {
		...
		BUG_ON(!(old_next == clear_flag(old_next)));
		BUG_ON(!(new_node != old_next));
		/*
		 ...
		 */
		BUG_ON(!(!is_removal_owner(old_next)));
	...
	BUG_ON(!(is_removed(CMM_LOAD_SHARED(old_node->next))));
	return 0;
}
		...
		int bucket_flag)
{
	...
	insert:
		BUG_ON(!(node != clear_flag(iter)));
		BUG_ON(!(!is_removed(iter_prev)));
		BUG_ON(!(!is_removal_owner(iter_prev)));
		BUG_ON(!(!is_removed(iter)));
		BUG_ON(!(!is_removal_owner(iter)));
		BUG_ON(!(iter_prev != node));
		if (!bucket_flag)
		...
		struct cds_lfht_node *node)
{
	...
	BUG_ON(!(is_removed(CMM_LOAD_SHARED(node->next))));
	/*
			...
			void *attr)
{
	...
	BUG_ON(!(ht));
	BUG_ON(!(ht->mm == mm));
	BUG_ON(!(ht->bucket_at == mm->bucket_at));

	ht->flags = flags;
	ht->flavor = flavor;
	ht->resize_attr = attr;
	/* this mutex should not nest in read-side C.S. */
	mutex_init(&ht->resize_mutex);
		...
		struct cds_lfht_iter *iter)
{
	...
	size = lockless_dereference(ht->size);
	bucket = lookup_bucket(ht, size, hash);
	/* We can always skip the bucket node initially */
	...
	for (;;) {
		...
		BUG_ON(!(node == clear_flag(node)));
		if (likely(!is_removed(next))
		    && !is_bucket(next)
		    && node->reverse_hash == reverse_hash
		    && likely(match(node, key))) {
				break;
		}
		node = clear_flag(next);
	}
	BUG_ON(!(!node || !is_bucket(CMM_LOAD_SHARED(node->next))));
	iter->node = node;
	iter->next = next;
}
		...
		struct cds_lfht_node *new_node)
{
	...
	if (!old_iter->node)
		return -ENOENT;
	if (unlikely(old_iter->node->reverse_hash != new_node->reverse_hash))
		return -EINVAL;
	if (unlikely(!match(old_iter->node, key)))
		return -EINVAL;
	size = lockless_dereference(ht->size);
...
int cds_lfht_del(struct cds_lfht *ht, struct cds_lfht_node *node)
{
	...
	size = lockless_dereference(ht->size);
	return _cds_lfht_del(ht, size, node);
}
